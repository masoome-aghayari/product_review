...
public interface CommentRepository extends PagingAndSortingRepository<Comment, Long> {
    @Query(value = """
            ...
            c.text, c.user.username, c.product.id, c.commentDate)
                                        FROM Comment c
                                        WHERE c.accepted = true AND c.product.id = :productId
                                        GROUP BY c.product.id, c.text, c.user.username, c.commentDate
                                        ORDER BY c.commentDate
            """)
    Page<CommentDto> getLatestCommentsForProduct(@Param("productId") Long productId, Pageable pageable);

    @Query(value = """
            SELECT count(c.id) FROM Comment c
            WHERE c.accepted = true AND c.product.id = :productId     
            """)
    Long countCommentByAcceptedIsTrueAndProductId(@Param("productId") Long productId);
